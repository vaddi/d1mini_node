...
// setup your default wifi ssid, passwd and dns stuff. Can overwriten later from setup website
...
char dnsname[eepromStringSize]  = "esp21";        // Uniq Networkname
...
bool bme280sensor               = true;           // enable/disable bme280 (on address 0x76). They use Pin 5 (SCL) and Pin 4 (SDA) for i2c
...
IPAddress ipaddr(192, 168, 1, 252); // Device IP Adress
...
bool loadSettings() {
  ...
  if( strcmp( checksum, data.eepromchecksum ) == 0 ) { // compare checksums
...
// Sting 2 IP Adress
// https://arduino.stackexchange.com/a/68559
//IPAddress str2ip( String ipStr ) {
...
//  if ( ! isIp(server.hostHeader()) && server.hostHeader() != (String(ESPHostname) + ".local" ) ) {
    ...
    server.sendHeader("Location", String("http://") + ip2Str( server.client().localIP() ) );
...
String htmlJS() {
  ...
  result += "window.setInterval( function() {"; // polling event to get wifi signal strength
  result += "  let xmlHttp = new XMLHttpRequest();\n";
  result += "  xmlHttp.open( 'GET', '/signal', false ); // false for synchronous request\n";
  ...
  result += "  function renderChannels() {\n";
  result += "    let mcpChannelsRow = document.getElementById('mcpChannelsRow');\n";
  result += "    let mcp3008sensor = document.getElementById('mcp3008sensor');\n";
  result += "    if( mcp3008sensor.checked == true ) {\n";
  result += "      mcpChannelsRow.style.display = 'contents';\n";
  result += "    } else {\n";
  result += "      mcpChannelsRow.style.display = 'none';\n";
  result += "    }\n";
  result += "  }\n";
  result += "  function renderStaticIPs() {";
  result += "    let staticIPRows = document.getElementsByClassName('staticIPRow');\n";
  result += "    let staticIP = document.getElementById('staticIP');\n";
  result += "    if( staticIP.checked == true ) {\n";
  result += "      for (i = 0; i < staticIPRows.length; i++) {\n";
  result += "        staticIPRows[i].style.display = 'table-row';\n";
  result += "      }\n";
  result += "    } else {\n";
  result += "      for (i = 0; i < staticIPRows.length; i++) {\n";
  result += "        staticIPRows[i].style.display = 'none';\n";
  result += "      }\n";
  result += "    }\n";
  result += "  }\n";
  result += "  window.onload = function () {\n";
  result += "    renderChannels();\n";
  result += "    renderStaticIPs();\n";
  result += "  };\n";
  result += "};\n";
  result += "</script>\n";
  return result;
...
String htmlCSS() {
  ...
  result += "}\n";
  result += "#mcpChannelsRow, .staticIPRow {\n";
  result += "  transition: all 500ms ease-out;\n";
  result += "  display: table-row;\n";
  result += "}\n";
  result += "@media only screen and (max-width: 620px) {\n";
  result += "  body { font-size: 200%; }\n";
  result += "  \n";
  result += "}\n";
  result += "</style>\n";
  return result;
...
void handle_root() {
  ...
  result += "Just add this Node to your prometheus.yml as Target to collect Air Pressure, ";
  result += "Air Humidity, Temperatures, Radiation and some Air Quality Data like Alcohole, ";
  ...
  result += "  <tr><td><a href='/restart'>/restart</a></td><td></td></tr>\n";
  result += "  <tr><td><a href='/restart?reset=1'>/reset</a></td><td>*Clear EEPROM, will reboot in Setup (Wifi Acces Point) Mode!</td></tr>\n";
  result += "</table>\n";
...
void setup() {
  ...
  // start Wifi depending on Load Config
  if( loadingSetup ) {
    // use a static IP?
    if( staticIP ) {
...
//    }
  ...
  server.on("/setup", HTTP_GET, []() {
    ...
    result += "    <td><input id='staticIP' name='staticIP' type='checkbox' onclick='renderStaticIPs()' " + staticIPChecked + " /></td>\n";
    result += "  </tr>\n";
    ...
    result += "    <td><input id='mcp3008sensor' name='mcp3008sensor' type='checkbox' " + mcp3008Checked + " onclick='renderChannels()' /></td>\n";
    result += "  </tr>\n";
    result += "  <tr id='mcpChannelsRow'>\n";
